                    ...
                    <tr>
                        ...
                        <td>
                            @foreach($produksi->pesanan->detailPesanan as $detail)
                                <span class="badge bg-light text-dark border">{{ $detail->produk->nama_produk }} ({{ $detail->jumlah }} {{ $detail->produk->satuan }})</span>
                            @endforeach
                        </td>
                        <td>
                            ...
                            @else
                                <span class="badge bg-primary rounded-pill px-3 text-capitalize">{{ $produksi->status_produksi }}</span>
                            @endif
                        </td>
                        <td class="text-end pe-4">
                            @if($produksi->status_produksi === 'antrian')
                                <form action="{{ route('produksi.ambil', $produksi) }}" method="POST" class="d-inline">
                                    @csrf
                                    <button type="submit" class="btn btn-sm btn-primary rounded-pill px-3">Ambil Pekerjaan</button>
                                </form>
                            @else
                                <a href="{{ route('produksi.show', $produksi) }}" class="btn btn-sm btn-outline-primary rounded-pill px-3">Update Status</a>
                            @endif
                        </td>
                    </tr>
                    @empty
