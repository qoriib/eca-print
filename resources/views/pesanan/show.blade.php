                                            ...
                                            <tr class="text-nowrap">
                                                <td class="ps-4">
                                                    ...
                                                    <small class="text-muted">{{ $detail->jumlah }}
                                                        {{ $detail->produk->satuan }}</small>
                                                </td>
                                                <td>
                                                    <div class="small text-muted">
                                                        @if($detail->ukuran)
                                                        <div>Size: {{ $detail->ukuran }}</div> @endif
                                                        @if($detail->bahan)
                                                        <div>Bahan: {{ $detail->bahan }}</div> @endif
                                                        @if($detail->finishing)
                                            ...
                                            @if($detail->file_desain)
                                                <tr class="bg-light-subtle">
                                                    <td colspan="4" class="ps-4 py-2">
                                                        <a href="{{ asset('storage/' . $detail->file_desain) }}" target="_blank"
                            ...
                            <div class="card h-100">
                                <div class="card-body p-4">
                                    <h6 class="fw-semibold mb-3"><i
                                            class="bi bi-chat-left-text me-2 text-primary"></i>Catatan Pelanggan</h6>
                                    <p class="small text-muted mb-0">
                                        {{ $pesanan->catatan_pelanggan ?? 'Tidak ada catatan.' }}</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="card h-100">
                                <div class="card-body p-4">
                                    <h6 class="fw-semibold mb-3"><i class="bi bi-reply-all me-2 text-primary"></i>Catatan
                                        Admin</h6>
                                    <p class="small text-muted mb-0">
                                        {{ $pesanan->catatan_admin ?? 'Belum ada catatan dari admin.' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="card-body p-4">
                            ...
                            @else
                                <div class="text-center py-3">
                                    ...
                                    @if(Auth::user()->role === 'pelanggan')
                                        <a href="{{ route('pembayaran.create', $pesanan) }}"
                                            class="btn btn-sm btn-outline-primary mt-3 px-4">Bayar Sekarang</a>
                                    @endif
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="modal-body">
                            <div class="mb-3">
                                ...
                                <select name="status" class="form-select">
                                    @foreach($steps as $key => $label)
                                        <option value="{{ $key }}" {{ $pesanan->status == $key ? 'selected' : '' }}>{{ $label }}
                                        </option>
                                    @endforeach
                            ...
                            <div class="mb-3">
                                ...
                                <input type="date" name="tanggal_deadline" class="form-control"
                                    value="{{ $pesanan->tanggal_deadline ? $pesanan->tanggal_deadline->format('Y-m-d') : '' }}">
                            </div>
                            <div class="mb-0">
                                <label class="form-label">Catatan Admin</label>
                                <textarea name="catatan_admin" class="form-control"
                                    rows="3">{{ $pesanan->catatan_admin }}</textarea>
                            </div>
                        </div>
