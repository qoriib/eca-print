            ...
            <div class="card overflow-hidden">
                <div class="list-group list-group-flush">
                    @forelse($notifikasi as $notif)
                        <div
                            class="list-group-item p-4 border-start border-4 {{ $notif->is_read ? 'border-light bg-white' : ($notif->tipe === 'sukses' ? 'border-success bg-success-subtle' : ($notif->tipe === 'peringatan' ? 'border-warning bg-warning-subtle' : 'border-primary bg-primary-subtle')) }}">
                                    ...
                                    <div class="mt-3 d-flex gap-2">
                                        @if(!$notif->is_read)
                                            <form action="{{ route('notifikasi.read', $notif) }}" method="POST">
                                                @csrf
                                                <button type="submit" class="btn btn-sm btn-link p-0 text-decoration-none">Tandai
                                                    Dibaca</button>
                                            </form>
                                        @endif
                                        <form action="{{ route('notifikasi.destroy', $notif) }}" method="POST"
                                            onsubmit="return confirm('Hapus notifikasi ini?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                class="btn btn-sm btn-link p-0 text-danger text-decoration-none">Hapus</button>
                                        </form>
                                    </div>
                                </div>
                            </div>
